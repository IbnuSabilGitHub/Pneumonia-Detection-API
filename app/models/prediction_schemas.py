...
class Probabilities(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"NORMAL": 0.92, "PNEUMONIA": 0.08}}
...
class ModelInfo(BaseModel):
    ...
    model_type: str = Field(
        ..., description="Type of neural network architecture used", example="standard"
    )
    model_version: str = Field(
        ..., description="Version identifier of the trained model", example="v1.0"
    )
...
class ValidationError(BaseModel):
    ...
    msg: str = Field(
        ..., description="Validation error message", example="Field required"
    )
    type: str = Field(..., description="Type of validation error", example="missing")
...
class PredictionConflictResponse(BaseErrorResponse):
    ...
    **Prediction Conflict Error Response**
    ...
    retry_after: Optional[int] = Field(
        None, description="Suggested wait time in seconds before retry", example=60
    )
...
class PredictionPayloadTooLargeResponse(BaseErrorResponse):
    ...
    **Prediction Payload Too Large Error Response**
    ...
    max_size_mb: float = Field(
        ..., description="Maximum allowed file size in megabytes", example=10.0
    )

    actual_size_mb: Optional[float] = Field(
        None, description="Actual uploaded file size in megabytes", example=15.2
    )
...
class PredictionRateLimitResponse(BaseErrorResponse):
    ...
    **Prediction Rate Limit Exceeded Response**
    ...
    limit: str = Field(
        ..., description="Applied rate limit description", example="5 per minute"
    )

    retry_after: int = Field(
        ..., description="Seconds to wait before next request", example=60
    )
...
class PredictionServiceUnavailableResponse(BaseErrorResponse):
    ...
    **Prediction Service Unavailable Error Response**
    ...
    service_status: Optional[str] = Field(
        None, description="Current service status", example="model_not_loaded"
    )

    retry_after: Optional[int] = Field(
        None, description="Suggested retry time in seconds", example=30
    )
