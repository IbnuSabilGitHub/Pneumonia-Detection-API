...
class StartupManager:
    ...
    async def initialize_rate_limiter(self, storage_config: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            storage_type = StorageType.MEMORY  # Default to memory for single-instance deployments
            ...
            if rate_limiter.storage:
                ...
                logger.info(f"✅ Rate limiter initialized with {storage_info.get('backend_type', 'unknown')} storage")
            else:
            ...
            try:
                ...
                self.warnings.append(f"Using fallback memory storage for rate limiter: {e}")
                logger.info("✅ Rate limiter initialized with fallback memory storage")
                return True
                
            except Exception as fallback_error:
                error_msg = f"Rate limiter initialization failed completely: {fallback_error}"
                logger.error(f"❌ {error_msg}")
                self.startup_errors.append(error_msg)
                return False
    ...
    async def run_health_checks(self) -> Dict[str, bool]:
        ...
        if 'prediction' in self.services:
            try:
                ...
                health_status['prediction_service'] = hasattr(service, 'model') and service.model is not None
            except Exception:
        ...
        if 'rate_limiter' in self.services:
            try:
                ...
                if limiter.storage:
                    ...
                    health_status['rate_limiter'] = True  # Memory storage always available
            except Exception:
    ...
    async def startup(self, storage_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if success_count == total_services:
            ...
            logger.warning(f"⚠️ {success_count}/{total_services} services initialized successfully")
        ...
        if 'prediction' in self.services:
            try:
                ...
                if hasattr(service, 'cleanup'):
                    ...
                    logger.info("✅ Prediction service cleanup completed (no cleanup needed)")
            except Exception as e:
