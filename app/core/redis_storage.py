...
class RedisStorageBackend(StorageBackend):
    ...
    async def _load_lua_scripts(self):
        ...
        try:
            self.script_hashes['increment'] = await self.redis_client.script_load(increment_script)
            self.script_hashes['list_append'] = await self.redis_client.script_load(list_append_script)
        except Exception as e:
    ...
    async def get_list(self, key: str) -> List[Any]:
        ...
        try:
            ...
            logger.error(f"Redis GET_LIST error for key {key}: {e}")
            return []
    
    async def append_to_list(self, key: str, value: Any, max_length: Optional[int] = None) -> bool:
        ...
        try:
            ...
            if 'list_append' in self.script_hashes and max_length:
                await self.redis_client.evalsha(
                    self.script_hashes['list_append'],
                    1,
                    redis_key,
                    value,
                    max_length
            ...
            logger.error(f"Redis APPEND_TO_LIST error for key {key}: {e}")
            return False
    ...
    async def get_hash(self, key: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"Redis GET_HASH error for key {key}: {e}")
            return {}
    ...
    async def set_hash_field(self, key: str, field: str, value: Any) -> bool:
        ...
        try:
            ...
            logger.error(f"Redis SET_HASH_FIELD error for key {key}, field {field}: {e}")
            return False
    ...
    async def cleanup_expired(self) -> int:
        ...
        try:
            ...
            logger.error(f"Redis CLEANUP_EXPIRED error: {e}")
            return 0
