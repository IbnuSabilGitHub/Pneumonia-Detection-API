...
class AdvancedRateLimiter:
    ...
    def __init__(
        ...
        self.file_hash_requests: Dict[str, List[Tuple[str, float]]] = defaultdict(
            list
        )  # file_hash: [(ip, timestamp)]
    ...
    def detect_ip_switching_attack(self, client_ip: str, fingerprint: str) -> bool:
        ...
        fingerprint_ips = [
            ...
            if any(
                fp.user_agent_hash == fingerprint
                for fp in self.ip_fingerprints.get(ip, [])
            )
        ]

        if (
            len(set(fingerprint_ips)) > 3
        ):  # Same fingerprint from 3+ different IPs (more strict)
            logger.warning(
    ...
    def calculate_global_attack_score(self) -> float:
        ...
        factors = {
            "high_request_rate": min(
                requests_per_minute / 100, 1.0
            ),  # Normalize to 0-1
    ...
    async def is_request_allowed(
        ...
        await self._append_to_storage_list(
            ip_fingerprints_key, fingerprint_data.to_dict(), max_length=100
        )
        ...
        if blocked_until and current_time < blocked_until:
            return (
                ...
                {"fingerprint": fingerprint, "unblock_time": blocked_until},
            )
        ...
        if ip_request_count > self.max_requests_per_ip:
            return (
                ...
                {"ip": client_ip, "requests_in_window": ip_request_count},
            )
        ...
        fingerprint_request_count = await self._get_from_storage(
            fingerprint_requests_key, 0
        )
        ...
        if attack_score > self.global_attack_threshold:
            ...
            if (
                ip_request_count > reduced_ip_limit
                or fingerprint_request_count > reduced_fingerprint_limit
            ):
                return (
                    False,
                    "High attack score - reduced limits applied",
                    {"attack_score": attack_score, "reduced_limits": True},
                )
